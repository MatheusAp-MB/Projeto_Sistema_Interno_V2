...
class GradePrecificacaoMagalu(models.Model):
    # * [EXPLICAÇÃO] → Espelha GradePrecificacaoML, mas SEM variacao/
    #                  tipo_anuncio — Magalu ainda não tem pipeline de
    #                  anúncio (só API do ML existe hoje), então a Grade é
    #                  só por PRODUTO. MargemGrade duplicada de propósito
    #                  (mesma decisão já tomada pro ML: só abstrai com 2+
    #                  casos reais na mão, não antes).

    produto = models.ForeignKey(
        'produtos.Produto', on_delete=models.CASCADE, related_name='grade_precificacao_magalu'
    )
    ...
    class Meta:
        ...
        verbose_name = 'Grade de Precificação Magalu'
        verbose_name_plural = 'Grade de Precificação Magalu'
