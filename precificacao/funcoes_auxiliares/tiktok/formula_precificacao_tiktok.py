# precificacao/funcoes_auxiliares/tiktok/formula_precificacao_tiktok.py

# Função Objetivo: Representa 1 margem candidata do TikTok Shop já resolvida, com auditoria.
# Explicação em detalhe: único marketplace fora do ML com "tipo" (Sem Afiliado/Com
# Afiliado) — a margem de afiliado (8%, confirmada) só entra na taxa quando
# tipo=com_afiliado. Comissão por faixa de preço reaproveita
# resolver_preco_por_faixa_comissao (mesma função do Shopee) — mas aqui o frete NÃO é
# fixo, vem de busca por peso (como o Magalu, sem faixa de reputação).
#
...
class DadosEntrada:
    ...
    tipo: str  # 'sem_afiliado' ou 'com_afiliado'
...
class FormulaPrecificacaoTiktok:
    ...
    def __init__(self, produto, config_tiktok, config_geral, margem_alvo_percentual, tipo,
                 faixas_comissao, frete_todas, faixas_armazenagem=None):
        self.produto = produto
        self.config_tiktok = config_tiktok
    ...
    # Função Objetivo: Busca o frete direto por peso — sem faixa de reputação (diferente do Magalu).
    def buscar_frete(self):
        ...
        self._frete = faixa.valor if faixa else None

    # Função Objetivo: Resolve o preço via busca de faixa de comissão (mesma função do Shopee).
    # Explicação em detalhe: margem de afiliado (8%) só soma na taxa quando
    # tipo=com_afiliado — é literalmente 0 quando tipo=sem_afiliado, sem nenhum if
    # espalhado no resto da fórmula (fica só aqui, num lugar só).
    def resolver_preco(self):
        if self._frete is None:
            self.resolvida = False
            return
        ...
        self._margem_afiliado_percentual = (
            self.config_tiktok.margem_afiliado_percentual if self.tipo == 'com_afiliado' else Decimal('0')
        )

        taxa_extra_fracao = (
            self._icms_saida_percentual + self._pis_saida_percentual
            + self._cofins_saida_percentual + self._margem_afiliado_percentual
        ) / 100
    ...
    def _montar_dados_intermediarios(self):
        self.intermediarios = DadosIntermediarios(
            ...
            comissao_percentual=self._comissao_percentual,
            comissao_valor=self._comissao_valor, margem_afiliado_valor=self._margem_afiliado_valor,
            adicional_fixo=self._adicional_fixo, fixo=self._fixo,
            taxa_percentual=self._taxa_percentual_fracao * 100,
            denominador=Decimal('1') - self._taxa_percentual_fracao - (self.margem_alvo_percentual / 100),
    ...
    def passos(self):
        ...
        return [
            ...
            {'ordem': 5, 'rotulo': 'Frete por peso', 'formula': f'peso {e.peso}kg, busca direta (sem faixa de reputação)', 'resultado': s.frete_usado},
            {'ordem': 6, 'rotulo': 'Faixa de comissão escolhida', 'formula': f'busca por preço, faixa R$ {i.faixa_comissao_preco_min}–{i.faixa_comissao_preco_max}', 'resultado': i.comissao_percentual},
            {'ordem': 7, 'rotulo': 'Adicional fixo da faixa', 'formula': 'valor fixo em R$ da mesma faixa de comissão', 'resultado': i.adicional_fixo},
            {'ordem': 8, 'rotulo': 'Margem de afiliado', 'formula': f"tipo='{e.tipo}' — só soma na taxa quando 'com_afiliado', senão é 0", 'resultado': i.margem_afiliado_valor},
            {'ordem': 9, 'rotulo': 'Taxa', 'formula': 'comissão da faixa + ICMS saída + PIS + COFINS + margem de afiliado', 'resultado': i.taxa_percentual},
            {'ordem': 10, 'rotulo': 'Denominador', 'formula': '1 − taxa − margem-alvo', 'resultado': i.denominador},
            {'ordem': 11, 'rotulo': 'Preço exato', 'formula': '(frete + adicional fixo + FIXO) ÷ denominador', 'resultado': i.preco_exato_antes_arredondar},
            {'ordem': 12, 'rotulo': 'Preço final (arredondado ,90)', 'formula': 'RoundUp90 — sempre pra CIMA', 'resultado': s.preco_final},
            {'ordem': 13, 'rotulo': 'Margem obtida', 'formula': 'preço×(1−taxa) − FIXO − frete − adicional fixo', 'resultado': s.margem_percentual_obtida},
            {'ordem': 14, 'rotulo': 'Preço de exibição (vitrine, decorativo)', 'formula': f'preço ÷ (1 − desconto vitrine {e.desconto_vitrine_percentual}%) — nunca usado em conta de margem', 'resultado': s.preco_de_exibicao},
        ]
