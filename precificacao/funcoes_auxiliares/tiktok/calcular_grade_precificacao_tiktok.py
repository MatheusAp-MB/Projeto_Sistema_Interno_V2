...
def _tipos():
    return ['sem_afiliado', 'com_afiliado']


def calcular_grade_precificacao_tiktok(stdout, style):
    from django.db import connection
    from produtos.models import Produto
    from tiktok.models import ConfiguracaoTiktok, FreteTiktok
    from precificacao.models import (
        ConfiguracaoOperacional, FaixaArmazenagem, TabelaComissaoTiktok, GradePrecificacaoTiktok,
    )

    stdout.write('[GRADE TIKTOK] Calculando...')
    ...
    existentes = {(r.produto_id, r.tipo, r.margem): r for r in GradePrecificacaoTiktok.objects.all()}
        ...
        for tipo in _tipos():
            for margem_chave, margem_valor in _margens():
                ...
                if existente:
                    ...
                    nova = GradePrecificacaoTiktok(produto=produto, tipo=tipo, margem=margem_chave, **dados)
    ...
    stdout.write(style.SUCCESS(
        f'[GRADE TIKTOK] Concluído em {tempo_total:.1f}s!\n'
