...
def calcular_grade_precificacao_raia(stdout, style):
    from django.db import connection
    from produtos.models import Produto
    from raia.models import ConfiguracaoRaia
    from precificacao.models import ConfiguracaoOperacional, FaixaArmazenagem, GradePrecificacaoRaia

    stdout.write('[GRADE RAIA] Calculando...')
    ...
    existentes = {(r.produto_id, r.margem): r for r in GradePrecificacaoRaia.objects.all()}
    ...
    for indice, produto in enumerate(produtos, start=1):
        ...
        for margem_chave, margem_valor in _margens():
            ...
            if existente:
                ...
                nova = GradePrecificacaoRaia(produto=produto, margem=margem_chave, **dados)
    ...
    if para_criar:
        GradePrecificacaoRaia.objects.bulk_create(para_criar, batch_size=BATCH_SIZE_PADRAO)
    if para_atualizar:
        GradePrecificacaoRaia.objects.bulk_update(
    ...
    stdout.write(style.SUCCESS(
        f'[GRADE RAIA] Concluído em {tempo_total:.1f}s!\n'
