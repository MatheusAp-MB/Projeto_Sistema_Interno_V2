...
def calcular_grade_precificacao_shopee(stdout, style):
    from django.db import connection
    from produtos.models import Produto
    from shopee.models import ConfiguracaoShopee
    from precificacao.models import (
        ConfiguracaoOperacional, FaixaArmazenagem, TabelaComissaoShopee, GradePrecificacaoShopee,
    )

    stdout.write('[GRADE SHOPEE] Calculando...')
    ...
    faixas_comissao = list(TabelaComissaoShopee.objects.all().order_by('preco_min'))

    existentes = {(r.produto_id, r.margem): r for r in GradePrecificacaoShopee.objects.all()}
        ...
        for margem_chave, margem_valor in _margens():
            try:
                formula = FormulaPrecificacaoShopee(
                    ...
                    margem_alvo_percentual=margem_valor, faixas_comissao=faixas_comissao,
            ...
            if existente:
                ...
                nova = GradePrecificacaoShopee(produto=produto, margem=margem_chave, **dados)
    ...
    if para_criar:
        GradePrecificacaoShopee.objects.bulk_create(para_criar, batch_size=BATCH_SIZE_PADRAO)
    if para_atualizar:
        GradePrecificacaoShopee.objects.bulk_update(
            para_atualizar,
            ['preco', 'margem_percentual_obtida', 'frete_usado', 'preco_de_exibicao', 'detalhamento'],
    ...
    stdout.write(style.SUCCESS(
        f'[GRADE SHOPEE] Concluído em {tempo_total:.1f}s!\n'
