...
def preparar_fixo_e_faixas(produto, frete_todas):
    ...
    produto e reaproveite nas 14 combinações (margem × Clássico/
    Premium × logística × catálogo) — nunca recalcular por combinação.
...
def calcular_preco_grade_ml(produto, config_tipo, margem_alvo_percentual, fixo, faixas_frete):
    """Retorna o dict de resolver_preco_por_margem() (preco_calculado,
    frete_usado, margem_percentual_obtida, faixa_frete) pra essa
    combinação de Produto + configuração + margem-alvo. Retorna None
    se a meta for inatingível, ou se nenhuma faixa de frete servir.
    ...
    fixo, faixas_frete: JÁ CALCULADOS por preparar_fixo_e_faixas() —
    não recalcula aqui, evita repetir a mesma conta/query 14x por
    produto.
    ...
    return resolver_preco_por_margem(
        fixo=fixo,
