...
#              Cada critério é um CriterioQualidadeSeed — não um dicionário
#              solto — pra garantir que os 5 campos sempre existem, com o
#              nome certo: erro de digitação vira aviso do editor antes
#              mesmo de rodar o comando, não um bug silencioso depois.

from dataclasses import dataclass
from mercado_livre.models import CriterioQualidade


# Função Objetivo: Representa 1 critério de qualidade conhecido, com forma
# fixa (5 campos, sempre esses) — nunca um dicionário solto.
@dataclass(frozen=True)
class CriterioQualidadeSeed:
    rule_key: str
    grupo: str
    nome: str
    pergunta: str
...
def popular_criterios_qualidade(stdout, style):
    ...
    for criterio in CRITERIOS:
        _, criado = CriterioQualidade.objects.update_or_create(
            rule_key=criterio.rule_key,
            defaults={
                'grupo': criterio.grupo,
                'nome': criterio.nome,
                'pergunta': criterio.pergunta,
                'como_aprovar': criterio.como_aprovar,
                'catalogado': True,
            }
        )
        stdout.write(f"    {criterio.rule_key}: {'criado' if criado else 'atualizado'}")