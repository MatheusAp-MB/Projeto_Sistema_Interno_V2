...
# maior entre físico e cúbico — regra oficial do Mercado Livre, replicada aqui.
...
# Função Objetivo: Diz se a variação tem as 4 dimensões declaradas pelo vendedor no ML.
def _variacao_tem_dimensao_completa(variacao):
    return (
        variacao is not None
        and variacao.altura_declarada_cm is not None
        and variacao.largura_declarada_cm is not None
        and variacao.comprimento_declarado_cm is not None
...
def resolver_dimensoes_efetivas(produto, variacao=None):
    if _variacao_tem_dimensao_completa(variacao):
        altura = variacao.altura_declarada_cm
        largura = variacao.largura_declarada_cm
        comprimento = variacao.comprimento_declarado_cm
